...
func (s *PromptService) CreatePrompt(ctx context.Context, name, template, model string, config map[string]interface{}) (*repository.Prompt, *domain.AppError) {
	if name == "" || template == "" {
	...
	prompt, err := s.repo.CreatePrompt(ctx, name, template, model, configBytes)
	if err != nil {
...
func (s *PromptService) ListPrompts(ctx context.Context, page, limit int) ([]repository.Prompt, *domain.AppError) {
	prompts, err := s.repo.ListPrompts(ctx, limit, (page-1)*limit)
	if err != nil {
...
func (s *PromptService) GetPrompt(ctx context.Context, name string) (*repository.Prompt, *domain.AppError) {
	prompt, err := s.repo.GetPrompt(ctx, name)
	if err != nil {
...
func (s *PromptService) RenderPrompt(ctx context.Context, name string, variables map[string]string) (*repository.Prompt, string, *domain.AppError) {
	prompt, err := s.repo.GetPrompt(ctx, name)
	if err != nil {
		return nil, "", domain.Wrap(domain.ErrInternal, 500, "failed to get prompt", err)
	}
...
func (s *PromptService) DeletePrompt(ctx context.Context, name string) *domain.AppError {
	if err := s.repo.DeletePrompt(ctx, name); err != nil {
