...
func (h *Handler) AdminReviewSuspicious(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.adminSvc.ReviewSuspicious(r.Context(), id, req.Action, adminID); err != nil { response.Error(w, 500, err.Error()); return }
...
func (h *Handler) AdminAddIPEntry(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&entry); err != nil { response.Error(w, 400, "Invalid body"); return }
	if err := h.adminSvc.AddIPEntry(r.Context(), &entry); err != nil { response.Error(w, 500, err.Error()); return }
	response.OK(w, entry)
}

func (h *Handler) AdminRemoveIPEntry(w http.ResponseWriter, r *http.Request) {
	if err := h.adminSvc.RemoveIPEntry(r.Context(), chi.URLParam(r, "id")); err != nil { response.Error(w, 500, err.Error()); return }
...
func (h *Handler) AdminStopImpersonation(w http.ResponseWriter, r *http.Request) {
	if err := h.adminSvc.EndImpersonation(r.Context(), chi.URLParam(r, "id")); err != nil { response.Error(w, 500, err.Error()); return }
...
func (h *Handler) AdminListAuditLogs(w http.ResponseWriter, r *http.Request) {
	...
	if v := r.URL.Query().Get("startDate"); v != "" {
		...
		if err == nil { filter.StartDate = &t }
	}
	if v := r.URL.Query().Get("endDate"); v != "" {
		t, err := time.Parse("2006-01-02", v)
		if err == nil { filter.EndDate = &t }
	}
...
func (h *Handler) AdminCreatePromoCode(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.adminSvc.CreatePromoCode(r.Context(), &p); err != nil { response.Error(w, 500, err.Error()); return }
	response.OK(w, p)
}
