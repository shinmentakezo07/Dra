...
const FAQ_WITH_CATEGORIES = pricingFAQ.map((item) => {
    ...
    if (q.includes("credit") || q.includes("expire") || q.includes("free tier") || q.includes("run out")) category = "credits";
    else if (q.includes("model") || q.includes("switch")) category = "models";
    return { ...item, category };
});

function FAQItem({ item, index }: { item: typeof pricingFAQ[0] & { category: string }; index: number }) {
                    ...
                    >
                        ...
                        <div className="relative rounded-2xl overflow-hidden">
                            <div
                                    ...
                                    style={{
                                        padding: "1px",
                                        background: "linear-gradient(180deg, rgba(139,92,246,0.3), rgba(236,72,153,0.1), rgba(139,92,246,0.05))",
                                        WebkitMask: "linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0)",
                                        WebkitMaskComposite: "xor",
                                        maskComposite: "exclude",
                                    }}
