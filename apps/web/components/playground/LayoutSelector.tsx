...
export default function LayoutSelector({ currentLayout, onLayoutChange }: LayoutSelectorProps) {
  ...
  const layouts: { value: LayoutMode; label: string; icon: typeof Columns; description: string }[] = [
      ...
      {isOpen && (
        <>
          <div
            ...
            onClick={() => { setIsOpen(false); setFocusIndex(-1); }}
          />
          <div className="absolute right-0 top-full mt-2 w-64 bg-[#1a1a1a] border border-white/10 rounded-lg shadow-2xl overflow-hidden z-50" role="listbox">
