...
const ACCENTS = {
  unified: { gradient: C.blue, border: "border-blue-500/20", text: "text-blue-400", glow: "bg-blue-500/10" },
  analytics: { gradient: C.blue, border: "border-blue-500/15", text: "text-blue-400", glow: "bg-blue-500/8" },
  edge: { gradient: C.purple, border: "border-purple-500/20", text: "text-purple-400", glow: "bg-purple-500/10" },
  routing: { gradient: C.amber, border: "border-amber-500/20", text: "text-amber-400", glow: "bg-amber-500/10" },
  pricing: { gradient: C.emerald, border: "border-emerald-500/20", text: "text-emerald-400", glow: "bg-emerald-500/10" },
};
...
function IconWrap({ accent, children, size = "default" }: { accent: typeof ACCENTS.unified; children: React.ReactNode; size?: "default" | "sm" }) {
...
function Pill({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/[0.03] border border-white/[0.06] text-[10px] font-mono font-semibold tracking-[0.15em] uppercase text-white/40 ${className}`}>
      {children}
    </span>
  );
}
  ...
  return (
    <div className="relative mt-6 w-full rounded-xl bg-black/60 border border-white/[0.06] p-4 font-mono text-xs shadow-2xl overflow-hidden">
      ...
      <div className="space-y-1.5 leading-relaxed">
        <p><span className="text-white/30">$</span> <span className="text-white/60">curl</span> <span className="text-blue-300">-X POST</span> <span className="text-green-300/70">https://api.yapa.up/v1/chat/completions</span></p>
        <p className="flex items-center gap-2 text-white/25"><span className="inline-block w-1.5 h-1.5 rounded-full bg-yellow-500/60 animate-pulse" /> routing to optimal model…</p>
        <p className="text-green-400/80 flex items-center gap-2"><span className="text-green-400">{"✓"}</span> 200 — claude-opus-4 (324 ms)</p>
        <p className="text-white/30"><span className="text-white/30">$</span> <span className="inline-block w-2 h-3.5 bg-green-400/60 animate-pulse" /></p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="mt-auto space-y-2">
      <svg className="w-full h-8" viewBox="0 0 200 32" preserveAspectRatio="none">
        ...
        <path d="M0 28 L8 26 L18 22 L30 24 L44 16 L58 19 L72 12 L86 14 L100 8 L114 10 L128 5 L144 7 L160 3 L176 5 L192 2 L200 3" fill="none" stroke="#3b82f6" strokeWidth="1.5" className="drop-shadow-[0_0_4px_rgba(59,130,246,0.3)]" />
        <path d="M0 28 L8 26 L18 22 L30 24 L44 16 L58 19 L72 12 L86 14 L100 8 L114 10 L128 5 L144 7 L160 3 L176 5 L192 2 L200 3 L200 32 L0 32 Z" fill="url(#sparkG)" />
      </svg>
      <div className="rounded-xl bg-white/[0.02] border border-white/[0.04] divide-y divide-white/[0.04]">
        {rows.map((r) => (
          <div key={r.label} className="flex items-center justify-between px-3 py-2">
            <span className="text-[10px] font-mono tracking-widest text-white/30">{r.label}</span>
...
function RoutingTags() {
  ...
  return (
    <div className="mt-4 flex flex-wrap gap-1.5">
      {tags.map((t) => (
        <span key={t} className="px-2.5 py-1 rounded-full bg-white/[0.03] border border-white/[0.06] text-[10px] font-mono font-medium text-white/40 tracking-wider">
          {t}
        </span>
      ))}
    </div>
...
function FeatureCard({ feature }: { feature: (typeof FEATURES)[number] }) {
  ...
  return (
    ...
    >
      ...
      <div className={`relative h-full bg-[#0A0A0A] rounded-2xl border ${feature.accent.border} border-white/[0.04] p-6 lg:p-8 flex flex-col overflow-hidden transition-colors duration-500`}>
        ...
        <div className="relative z-10 flex flex-col h-full">
          <div className="flex items-start justify-between">
            <IconWrap accent={feature.accent} size={feature.visual === "terminal" ? "default" : "sm"}>
              <Icon className={feature.visual === "terminal" ? "w-6 h-6" : "w-5 h-5"} />
            </IconWrap>
            <ArrowUpRight className="w-4 h-4 text-white/20 group-hover:text-white/60 transition-colors duration-300" />
          </div>

          <h3 className="mt-4 text-xl lg:text-2xl font-bold text-white tracking-tight">{feature.title}</h3>
          <p className="mt-2 text-sm text-white/50 leading-relaxed max-w-md">{feature.desc}</p>
...
export function GatewayFeatures() {
  ...
  return (
    <section ref={ref} className="relative w-full py-24 lg:py-40 px-4 overflow-hidden">
      {/* Parallax atmosphere */}
      <motion.div
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div className="mb-16 lg:mb-24">
          ...
          >
            Route requests to GPT-4o, Claude Opus 4.5, Gemini 2.5 Pro, Llama 4, and 100+ models through a single OpenAI-compatible endpoint. No SDK lock-in, no migration pain.
          </motion.p>
        </div>
