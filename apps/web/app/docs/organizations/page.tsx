...
export default function OrganizationsPage() {
  return (
    ...
    >
      <Section id="organizations" icon={Users} title="Organizations" accent="amber">
        <p>
          Organizations enable teams to collaborate with shared resources, API keys, and credit pools.
          Each organization has an owner (creator) who can invite members, manage roles, and control
          access. Members share the organization's credits and can use its API keys.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
          {[
            { title: "Shared Credits", icon: Shield, desc: "All organization members draw from a common credit balance. Track spending per member." },
  ...
  "createdAt": "2026-05-13T10:00:00Z"
        ...
        <h3 className="text-lg font-bold text-white mb-4 mt-10">Managing Organizations</h3>
        <div className="space-y-2">
          <EndpointCard method="GET" path="/api/organizations" description="List all organizations you own or are a member of. Returns both owned and joined organizations, deduplicated by ID." />
          <EndpointCard method="POST" path="/api/organizations" description="Create a new organization. You become the owner with admin role. Name must be at least 2 characters.">
            <CodeBlock
              language="json"
  ...
  -d '{
    "name": "My Engineering Team"
          ...
          <EndpointCard method="GET" path="/api/organizations/{id}" description="Get organization details including name, owner, plan tier, and creation date." />
          <EndpointCard method="GET" path="/api/organizations/{id}/members" description="List all members with their user IDs, roles, and join dates.">
            <CodeBlock
              language="json"
              code={`// Response
[
  ...
  },
  {
    ...
    "joinedAt": "2026-05-13T11:00:00Z"
        ...
        <div className="p-5 rounded-xl bg-white/[0.01] border border-white/[0.05] mb-6">
          ...
          <ol className="space-y-3 text-sm text-white/30">
            ...
            <li className="flex items-start gap-3">
              ...
              <span>System creates an invitation with a unique token and sends an email with the accept link (if SMTP is configured).</span>
            </li>
            <li className="flex items-start gap-3">
        ...
        <div className="space-y-2">
          <EndpointCard method="POST" path="/api/organizations/{id}/invite" description="Invite a user by email with a specific role. An invitation email is sent if SMTP is configured. Returns the invite with a unique token.">
            <CodeBlock
              language="json"
  ...
  -d '{
    ...
    "role": "member"
          ...
          <EndpointCard method="POST" path="/api/invites/accept" description="Accept a pending organization invitation using the token from the invite email.">
            <CodeBlock
              language="json"
  ...
  -d '{
    "token": "invite_token_from_email"
          ...
          <EndpointCard method="POST" path="/api/organizations/{id}/members/{userId}" description="Remove a member from the organization. Only admins can remove members. The owner cannot be removed." />
        </div>

        <h3 className="text-lg font-bold text-white mb-4 mt-10">Roles & Permissions</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.05]">
                ...
                <th className="text-left py-3 px-4 text-white/40 font-medium text-xs uppercase tracking-wider">Use Credits</th>
                <th className="text-left py-3 px-4 text-white/40 font-medium text-xs uppercase tracking-wider">Delete Org</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.04]">
              {[
                { role: "Owner", invite: "Yes", remove: "Yes", use: "Yes", delete: "Yes" },
        ...
        <TipBox>
          Organization credits and API keys are separate from your personal account. When you create or join an organization,
          switch between contexts in the dashboard to access shared resources. Only the owner can delete an organization.
        </TipBox>
      </Section>
    </motion.div>
  );
