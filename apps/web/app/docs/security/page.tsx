  ...
  return (
    ...
    >
      <Section id="security" icon={Lock} title="Security" accent="violet">
        <p>
          Security is built into every layer of Yapapa. All data in transit is encrypted via TLS. API keys are hashed using HMAC-SHA256 before storage — the raw key is never persisted.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
          {[
            { title: "Encryption in Transit", desc: "TLS 1.3 for all API endpoints. All communication between clients and servers is encrypted.", icon: Lock },
            { title: "Key Hashing", desc: "API keys stored as HMAC-SHA256 hashes with a server-side pepper. The raw key token is returned once at creation and never stored.", icon: Key },
            { title: "Rate Limiting", desc: "Per-user sliding window rate limits with configurable RPM. Stricter limits on auth endpoints (10 req/min).", icon: Activity },
            { title: "CORS Protection", desc: "Strict CORS policy. Only allowed origins can make browser requests to the API.", icon: Globe },
          ].map((item) => (
...
const storedHash = hashAPIKey(providedKey, AUTH_SECRET);`}
        ...
        <div className="mt-10">
          ...
          <p className="text-sm text-white/40 mb-4">
            The platform sets security headers on all responses to protect against common web vulnerabilities.
          </p>
          <CodeBlock
            language="bash"
...
Permissions-Policy: camera=(), microphone=(), geolocation=()`}
        ...
        <TipBox variant="warning">
          API keys are shown only once at creation. Store them securely — if lost, you must revoke and regenerate. The <code className="text-blue-400 font-mono text-xs">AUTH_SECRET</code> environment variable must be identical in both frontend and backend deployments for JWT validation to work.
        </TipBox>
      </Section>
    </motion.div>
  );
