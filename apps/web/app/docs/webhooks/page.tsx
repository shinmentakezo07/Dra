...
export default function WebhooksPage() {
    ...
    >
      <Section id="webhooks" icon={Webhook} title="Webhooks">
        <p>
          ...
          and other platform events. Each webhook can filter specific event types and includes
          HMAC-signed payloads for authenticity verification.
        </p>

        <h3 className="text-lg font-bold text-white mb-4 mt-8">Available Events</h3>
        ...
        <div className="mt-6">
          <h4 className="text-white font-semibold text-sm mb-3">Example Payload</h4>
          <CodeBlock
            language="json"
            code={`{
  ...
  -d '{
    ...
    "headers": {
      "Authorization": "Bearer my-custom-token"
          ...
          <EndpointCard method="GET" path="/api/webhooks/{id}" description="Get a single webhook endpoint configuration, including its event subscriptions, custom headers, active status, and delivery history." />
          <EndpointCard method="PUT" path="/api/webhooks/{id}" description="Update an existing webhook endpoint. Accepts the same fields as Create. Supports partial updates to URL, events, secret, and headers.">
            <CodeBlock
              language="json"
  ...
  -d '{
    ...
    "headers": {}
          ...
          <EndpointCard method="DELETE" path="/api/webhooks/{id}" description="Permanently delete a webhook endpoint. All pending deliveries are cancelled and no further events will be sent." />
        </div>
...
function verifyWebhook(payload, signature, secret) {
  ...
  return crypto.timingSafeEqual(
    ...
    Buffer.from(expected)
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            { title: "Event Filtering", desc: "Subscribe only to specific event types. Use the events array when creating a webhook. Unsubscribed events are silently ignored." },
            { title: "Custom Headers", desc: "Attach custom HTTP headers to every delivery. Useful for bearer tokens or API keys your endpoint requires." },
        ...
        <TipBox>
          Always verify the <code className="text-blue-400 font-mono text-xs">X-Webhook-Signature</code> header using your webhook secret before processing incoming webhooks.
