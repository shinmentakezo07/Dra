  ...
  return (
    ...
    >
      <Section id="rate-limits" icon={Shield} title="Rate Limits" accent="amber">
        <p>
          Rate limits protect the API from abuse and ensure fair usage. Limits are applied per-user based on API key or session, using a sliding window algorithm. The default limits can be configured via environment variables.
        </p>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
          {[
            { label: "Requests per minute", value: "60", desc: "Sliding window per API key", icon: Gauge },
            { label: "Tokens per minute", value: "100K", desc: "Combined input + output tokens", icon: Timer },
            { label: "Concurrent requests", value: "10", desc: "Simultaneous connections per user", icon: Users },
          ].map((stat) => (
        ...
        <div className="mt-8">
          ...
          <p className="text-sm text-white/40 mb-4">
            Every response includes headers that indicate your current rate limit status. These can be used to implement client-side rate limiting.
          </p>
          <CodeBlock
            language="bash"
...

{
  ...
  "message": "Rate limit exceeded. Try again in 2 seconds."
        ...
        <div className="mt-8">
          ...
          <p className="text-sm text-white/40 mb-4">
            Different endpoint groups have different rate limits. Auth endpoints have stricter limits to prevent brute force attacks.
          </p>
          <div className="overflow-x-auto mt-4">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/[0.05]">
                  <th className="text-left py-3 px-4 text-white/40 font-medium text-xs uppercase tracking-wider">Group</th>
                  <th className="text-left py-3 px-4 text-white/40 font-medium text-xs uppercase tracking-wider">Limit</th>
                  <th className="text-left py-3 px-4 text-white/40 font-medium text-xs uppercase tracking-wider">Window</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/[0.04]">
                {[
                  { group: "Auth (login, signup)", limit: "10", window: "1 minute" },
                  { group: "API endpoints", limit: "60", window: "1 minute" },
                  { group: "OpenAI-compatible proxy", limit: "60", window: "1 minute" },
                  { group: "Admin endpoints", limit: "120", window: "1 minute" },
                ].map((row) => (
                  <tr key={row.group} className="text-white/30 text-xs hover:bg-white/[0.01] transition-colors">
        ...
        <TipBox>
          Rate limits are applied per-user based on API key or session. When using Redis (via <code className="text-blue-400 font-mono text-xs">REDIS_URL</code>), rate limit state is shared across instances for distributed rate limiting. Contact support for higher limits on paid plans.
        </TipBox>
      </Section>
    </motion.div>
  );
