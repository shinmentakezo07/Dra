...
const itemVariants = {
  ...
  visible: { opacity: 1, y: 0, transition: { type: "spring" as const, stiffness: 110, damping: 22 } },
};
    ...
    >
      ...
      <div className="relative z-10 space-y-3.5">
        ...
        <div>
          ...
          <p className="text-[11px] font-semibold tracking-[0.12em] uppercase text-[var(--admin-text-muted)] mt-1">{label}</p>
          {sub && <p className="text-[12px] text-[var(--admin-text-dim)] mt-0.5">{sub}</p>}
        </div>
      </div>
    </motion.div>
  );
...
function QuickActionCard({ href, label, icon: Icon, description }: { href: string; label: string; icon: React.ElementType; description: string }) {
  return (
    <Link
      href={href}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-[var(--admin-text-dim)]">{description}</p>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Recent Users */}
        <motion.div variants={itemVariants} className="lg:col-span-2 admin-card p-6">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-[15px] font-semibold text-[var(--admin-text)] tracking-[-0.01em]">Recent Users</h2>
              <p className="text-[11px] text-[var(--admin-text-dim)] mt-0.5 font-mono">Latest registrations</p>
            </div>
            <Link
              href="/admin/users"
          ...
          <div className="space-y-0.5">
            {usersData?.data?.slice(0, 5).map((user: AdminUserDetail, i: number) => (
              <motion.div
                key={user.id}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-[13px] text-[var(--admin-text)] truncate font-medium">{user.name || "—"}</p>
                    <span className="admin-badge bg-white/[0.03] text-[var(--admin-text-dim)] border border-white/[0.04] px-1.5 py-[1px] text-[9px]">
                      {user.role}
                    </span>
                  </div>
                  <p className="text-[11px] text-[var(--admin-text-dim)] truncate font-mono">{user.email}</p>
                </div>
            ...
            {(!usersData?.data || usersData.data.length === 0) && (
              <p className="text-[13px] text-[var(--admin-text-dim)] text-center py-8">No users registered yet</p>
            )}
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="admin-card p-6">
          <div className="mb-5">
            <h2 className="text-[15px] font-semibold text-[var(--admin-text)] tracking-[-0.01em]">Quick Actions</h2>
            <p className="text-[11px] text-[var(--admin-text-dim)] mt-0.5 font-mono">Common admin tasks</p>
          </div>
          <div className="space-y-2">
            <QuickActionCard href="/admin/users" label="Manage Users" icon={Users} description="View and manage accounts" />
            <QuickActionCard href="/admin/providers" label="View Providers" icon={Server} description="Configure AI backends" />
            <QuickActionCard href="/admin/models" label="Browse Models" icon={Activity} description="Model registry & aliases" />
            <QuickActionCard href="/admin/billing" label="Billing" icon={DollarSign} description="Revenue & adjustments" />
          </div>
        </motion.div>
      </div>
    </motion.div>
