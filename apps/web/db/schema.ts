...
export const users = pgTable("users", {
  ...
  rateLimitTierIdx: index("idx_users_rate_limit_tier").on(table.rateLimitTierId),
...
export const conversationMessages = pgTable("conversation_messages", {
  ...
  conversationIdIdx: index("idx_conversation_messages_conv_id").on(table.conversationId),
...
export const prompts = pgTable("prompts", {
  ...
  nameVersionUnique: uniqueIndex("idx_prompts_name_version").on(table.name, table.version),
}));
...
export const orgMembers = pgTable("org_members", {
  ...
  orgUserUnique: uniqueIndex("idx_org_members_org_user").on(table.orgId, table.userId),
}));
...
export const providers = pgTable("providers", {
  ...
  circuitBreakerRecoveryMs: integer("circuit_breaker_recovery_ms").default(30000).notNull(),
  circuitBreakerHalfOpenMax: integer("circuit_breaker_half_open_max").default(3).notNull(),
...
export const providerKeys = pgTable("provider_keys", {
  ...
  monthlyQuota: bigint("monthly_quota", { mode: "number" }).default(0).notNull(),
  monthlyUsed: bigint("monthly_used", { mode: "number" }).default(0).notNull(),
  ...
  activeIdx: index("idx_provider_keys_active").on(table.providerId, table.isActive),
  strategyIdx: index("idx_provider_keys_strategy").on(table.strategy),
  sortOrderIdx: index("idx_provider_keys_sort_order").on(table.providerId, table.sortOrder),
}));
...
export const providerKeyUsageLogs = pgTable("provider_key_usage_logs", {
  ...
  requestIdCreatedAtIdx: uniqueIndex("idx_pk_usage_request_id").on(table.requestId, table.createdAt),
...
export const providerHealthChecks = pgTable("provider_health_checks", {
  ...
  providerIdx: index("idx_provider_health_provider").on(table.providerId, table.checkedAt),
}));
...
export const modelRegistry = pgTable("model_registry", {
  ...
  inputPricePer1k: numeric("input_price_per_1k", { precision: 12, scale: 8 }).default("0").notNull(),
  outputPricePer1k: numeric("output_price_per_1k", { precision: 12, scale: 8 }).default("0").notNull(),
...
export const modelAliases = pgTable("model_aliases", {
  ...
  targetModelId: uuid("target_model_id").references(() => modelRegistry.id).notNull(),
...
export const creditAdjustments = pgTable("credit_adjustments", {
  ...
  userIdx: index("idx_credit_adjustments_user").on(table.userId, table.createdAt),
}));
...
export const usageRecords = pgTable("usage_records", {
  ...
  requestIdCreatedAtIdx: uniqueIndex("idx_usage_records_request_id").on(table.requestId, table.createdAt),
  userIdx: index("idx_usage_records_user").on(table.userId, table.createdAt),
  providerIdx: index("idx_usage_records_provider").on(table.providerId, table.createdAt),
}));
...
export const usageDaily = pgTable("usage_daily", {
  ...
  primaryKey: primaryKey({ columns: [table.date, table.userId, table.modelId, table.apiKeyId] }),
}));
...
export const ipAccessLogs = pgTable("ip_access_logs", {
  ...
  blockedIdx: index("idx_ip_access_blocked").on(table.blocked, table.createdAt),
}));
...
export const suspiciousActivities = pgTable("suspicious_activities", {
  ...
  categoryIdx: index("idx_suspicious_category").on(table.category, table.createdAt),
  userIdx: index("idx_suspicious_user").on(table.userId),
  reviewedIdx: index("idx_suspicious_reviewed").on(table.reviewed, table.resolved),
}));
...
export const promoCodes = pgTable("promo_codes", {
  ...
  activeIdx: index("idx_promo_codes_active").on(table.isActive, table.expiresAt),
}));

export const promoRedemptions = pgTable("promo_redemptions", {
  id: uuid("id").defaultRandom().primaryKey(),
  promoId: uuid("promo_id").references(() => promoCodes.id).notNull(),
  userId: uuid("user_id").references(() => users.id).notNull(),
  discount: integer("discount").default(0).notNull(),
  creditsAwarded: integer("credits_awarded").default(0).notNull(),
  redeemedAt: timestamp("redeemed_at").defaultNow().notNull(),
}, (table) => ({
  promoUserUnique: uniqueIndex("idx_promo_redemptions_unique").on(table.promoId, table.userId),
}));
...
export const providerAbTests = pgTable("provider_ab_tests", {
  id: uuid("id").defaultRandom().primaryKey(),
  name: text("name").notNull(),
  providerA: uuid("provider_a").references(() => providers.id).notNull(),
  providerB: uuid("provider_b").references(() => providers.id).notNull(),
...
export const providerSla = pgTable("provider_sla", {
  ...
  providerDateUnique: uniqueIndex("idx_provider_sla_unique").on(table.providerId, table.date),
}));
...
export const providerMaintenanceWindows = pgTable("provider_maintenance_windows", {
  id: uuid("id").defaultRandom().primaryKey(),
  providerId: uuid("provider_id").references(() => providers.id).notNull(),
  title: text("title").notNull(),
  description: text("description").default(""),
